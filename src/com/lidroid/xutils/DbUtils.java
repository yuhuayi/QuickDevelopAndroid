...
public class DbUtils {
	...
	public void saveOrUpdateInThread(final Object entity) {
	...
	public void saveOrUpdateAll(List<?> entities) throws DbException {
		...
		try {
			...
			for (int i = 0; entities != null && i < entities.size(); i++) {
				saveOrUpdateWithoutTransaction(entities.get(i));
			}
			setTransactionSuccessful();
		} finally {
			endTransaction();
	...
	public void saveOrUpdateAllinThread(final List<?> entities) {
		ThreadPoolManager.getInstance().addTask(new Runnable() {
			@Override
			public void run() {
				if (entities == null || entities.size() == 0)
			...
			public void run() {
				try {
					...
					for (int i = 0; i < entities.length; i++) {
						List<?> list = entities[i];
						if (list == null || list.size() == 0)
							continue;
						createTableIfNotExist(list.get(0).getClass());
						for (int j = 0; entities != null && j < list.size(); j++) {
							saveOrUpdateWithoutTransaction(list.get(j));
						}
					}
					setTransactionSuccessful();
				} catch (DbException e) {
	...
	public void deleteAllInThread(final Class<?> entityType) {
		ThreadPoolManager.getInstance().addTask(new Runnable() {
